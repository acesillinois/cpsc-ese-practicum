...
###
### RQ1: pollination deficits for seed set, in relation to cultivar
###
...
variables_with_na <- c('Data_ID', 'Study_ID', 'Cultivar', 'Location_ID', 'SS_open', 'SS_supp', 'HB', 'WB', 'Richness')
...
# subset desired data -
data2 = data.frame(Data_ID, Study_ID, SS_open, SS_supp)
attach(data2)
str(data2)

# Wide to long
data3 <- pivot_longer(data2, cols = "SS_open":"SS_supp", names_to = "SS_treat",
                      values_to = "SS", values_drop_na = TRUE)
attach(data3)
str(data3)
dotchart(SS)
...
data4 <- data3 %>%
  group_by(Study_ID)%>%
  mutate(zSS=(SS-min(SS))/(max(SS)-min(SS)))
attach(data4)
str(data4)
dotchart(zSS)

# long to wide
data5 = data.frame(Data_ID, SS_treat, zSS)
attach(data5)
str(data5)
data6 <- pivot_wider(data = data5, id_cols = c("Data_ID"), 
                     names_from = "SS_treat", values_from = "zSS")
attach(data6)
str(data6)

### Link datasets with each other
data_1 = data1 %>% select(Data_ID, Study_ID, Cultivar, Location_ID, HB, WB, Richness)
data_2 = data6 %>% select(Data_ID, SS_open, SS_supp)
# Join the datasets 
...
merged_data2 <- merged_data %>%  
  select(Cultivar, zSS) %>% 
  group_by(Cultivar) %>% 
  summarise(across(zSS, .fns = se))

### Run pollen limitation model
# select variables
# response
SS_lim = merged_data$zSS
# fixed and random
...
# Run model
m1 = lme(SS_lim ~ -1 + cultivar, random=~1|study/location)
#m1 = lmer(SS_lim ~ -1 + cultivar + (1|study/location))
...
ggplot(data=data1, aes(x = Cultivar, col = Cultivar)) +
  geom_jitter(aes(y=SS_lim), size = 1, alpha = 0.25,height=0.01, width=0.1)+
  ...
  scale_y_discrete("Seed set deficit")+
  theme_classic()+
  theme(axis.text.x = element_text(angle = 45, hjust=1))
ggsave("RQ1_SSdeficit.png", width = 3, height = 3, dpi = 300)
...
###
### RQ2: pollinadtion deficit seed set ~ bee visitation
###
...
ggplot(data=merged_data3, aes(x=var))+
  ...
  ylab("Seed set deficit")+
  labs(subtitle="Bluecrop")+
  guides(color = FALSE, size = FALSE)+
  theme_classic()
ggsave("RQ2_SS_BC.png", width = 3.75, height = 3.75, dpi = 300)
